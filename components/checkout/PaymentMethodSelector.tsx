...
import { CheckCircle2, Info, PiggyBankIcon, Sparkles, Star } from "lucide-react";
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="bg-chart-3/20 border-chart-3 border-2 rounded-2xl p-6 shadow-md mb-6 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div>
            <h4 className="font-bold text-lg mb-2 text-chart-3/90">Hướng dẫn thanh toán</h4>
            <p className="text-base text-foreground">
              Vui lòng chọn phương thức thanh toán và hoàn tất giao dịch trước khi bấm "Hoàn tất đơn hàng".
              <span className="block mt-2 p-2 bg-white/50 rounded-lg border border-chart-3/30">
                <span className="font-bold">Lưu ý:</span> Chỉ <span className="font-bold underline decoration-wavy decoration-chart-3/70">sau khi đã thanh toán</span>, hãy đánh dấu vào ô xác nhận ở dưới.
              </span>
            </p>
          </div>
        </div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              <span className="font-bold text-lg block">Chuyển khoản ngân hàng</span>
              <span className="text-sm text-muted-foreground">Chuyển tiền qua tài khoản ngân hàng</span>
            </div>
            <span className="inline-flex items-center justify-center px-3 py-1 rounded-full text-sm font-semibold bg-primary text-primary-foreground ml-auto">Khuyên dùng</span>
          </div>
        </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white p-3 rounded-xl border border-blue-200">
                    <p className="text-sm text-blue-500 font-medium mb-1">Ngân hàng:</p>
                    <p className="font-bold text-lg">Vietcombank</p>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-blue-200">
                    <p className="text-sm text-blue-500 font-medium mb-1">Số tài khoản:</p>
                    <p className="font-bold text-lg">1234567890</p>
                  </div>
                </div>
                
                <div className="bg-white p-3 rounded-xl border border-accent">
                  <p className="text-sm text-accent-foreground font-medium mb-1">Chủ tài khoản:</p>
                  <p className="font-bold text-lg">CÔNG TY CỔ PHẦN HAVAMATH</p>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white p-3 rounded-xl border border-accent">
                    <p className="text-sm text-accent-foreground font-medium mb-1">Chi nhánh:</p>
                    <p className="font-bold">Hồ Chí Minh</p>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-accent/50">
                    <p className="text-sm text-accent-foreground font-medium mb-1">Số tiền:</p>
                    <p className="font-bold text-destructive">Chính xác số tiền đơn hàng</p>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-3 mt-2">
                  <div className="flex items-start gap-3 bg-white p-3 rounded-xl shadow-sm">
                    <div className="bg-primary text-white rounded-full h-7 w-7 flex items-center justify-center font-bold flex-shrink-0">1</div>
                    <p className="font-semibold">Chuyển khoản theo thông tin bên trên</p>
                  </div>
                  
                  <div className="flex items-start gap-3 bg-white p-3 rounded-xl shadow-sm">
                    <div className="bg-primary text-white rounded-full h-7 w-7 flex items-center justify-center font-bold flex-shrink-0">2</div>
                    <p className="font-semibold">Chụp ảnh hoặc lưu biên lai chuyển khoản</p>
                  </div>
                  
                  <div className="flex items-start gap-3 bg-white p-3 rounded-xl shadow-sm">
                    <div className="bg-primary text-white rounded-full h-7 w-7 flex items-center justify-center font-bold flex-shrink-0">3</div>
                    <p className="font-semibold">Đánh dấu vào ô xác nhận đã thanh toán bên dưới</p>
                  </div>
                  
                  <div className="flex items-start gap-3 bg-white p-3 rounded-xl shadow-sm">
                    <div className="bg-primary text-white rounded-full h-7 w-7 flex items-center justify-center font-bold flex-shrink-0">4</div>
                    <p className="font-semibold">Nhấn nút "Hoàn tất đơn hàng"</p>
                  </div>
                </div>
                ...
                <div className="bg-chart-3/10 p-3 rounded-xl mt-2 border border-chart-3/30">
                  <p className="font-semibold flex items-center">
                    ...
                    Sau khi xác nhận thanh toán, chúng tôi sẽ kích hoạt khóa học trong vòng 24 giờ làm việc.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              ...
              <span className="text-sm text-muted-foreground">Thanh toán qua ví điện tử MoMo</span>
            </div>
          </div>
        </div>
          ...
          <div className="mt-6 pl-4 sm:pl-10 space-y-6">
            ...
            <div className="bg-chart-5/10 rounded-2xl p-5 border-2 border-chart-5 shadow-md transition-shadow relative">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white p-3 rounded-xl border border-chart-5">
                    <p className="text-sm text-chart-5 font-medium mb-1">Số điện thoại MoMo:</p>
                    <p className="font-bold text-lg">0987654321</p>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-chart-5">
                    <p className="text-sm text-chart-5 font-medium mb-1">Tên tài khoản:</p>
                    <p className="font-bold text-lg">NGUYEN VAN A</p>
                  </div>
                </div>
                
                <div className="bg-white p-3 rounded-xl border border-chart-5/50">
                  <p className="text-sm text-chart-5 font-medium mb-1">Số tiền:</p>
                  <p className="font-bold text-destructive">Chính xác số tiền đơn hàng</p>
                </div>
              ...
              <div className="mt-6 bg-white p-4 rounded-xl border border-chart-5/30 flex items-start gap-3">
                ...
                <p className="text-sm">
                  <span className="font-bold">Tip:</span> Khi quét mã QR hoặc thanh toán qua MoMo, hãy nhấn vào nút "Gởi tới bạn bè" và nhập số điện thoại bên trên.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 border-2 border-chart-5 shadow-md hover:shadow-lg transition-shadow">
              <h3 className="font-bold text-lg text-chart-5 mb-3">Ðược phát triển bởi</h3>
              <div className="flex justify-center items-center py-6 px-4 bg-chart-5/5 rounded-xl border border-chart-5/20">
                <div className="text-center">
                  <div className="text-4xl font-bold mb-2 text-chart-5">MoMo</div>
                  <p className="text-sm text-muted-foreground">Ví điện tử hàng đầu Việt Nam</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start gap-4">
              <div className="flex-shrink-0">
                <div className="bg-primary/10 rounded-full p-3">
                  <CheckCircle2 className={`w-8 h-8 ${isPaymentConfirmed ? 'text-primary' : 'text-gray-300'} transition-colors duration-300`} />
                </div>
              </div>
              
              <div className="flex-1">
                <h3 className="font-bold text-xl mb-3 text-primary">Xác nhận thanh toán</h3>
                ...
                <div className="bg-primary/5 p-4 rounded-xl border border-primary/20 mb-4">
                  <div className="flex items-center gap-3 mb-3">
                    <input 
                      ...
                      onChange={(e) => handlePaymentConfirmChange(e.target.checked)}
                      className="h-5 w-5 text-primary border-primary rounded focus:ring-primary cursor-pointer"
                    />
                    <label htmlFor="payment-confirmed" className="font-bold text-lg cursor-pointer text-primary">
                      Tôi đã hoàn tất thanh toán
                    </label>
                  </div>
                  
                  <p className="text-base ml-8">
                    Tôi xác nhận đã thanh toán đầy đủ số tiền theo hướng dẫn và hiểu rằng khóa học sẽ được kích hoạt sau khi đơn vị xác nhận thanh toán thành công.
                  </p>
                </div>
